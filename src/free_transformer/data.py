"""Tokenization and data loading (ported from mla-gpt for comparability).
...
sampled as random contiguous windows, nanoGPT-style, via a fresh memmap per
call to avoid leaking page cache.
"""
...
def make_get_batch(data_dir: str | Path, block_size: int, device: str = "cuda"):
    ...
    dev = str(device)
    pin = dev.startswith("cuda")  # pinned-memory fast path is CUDA-only
